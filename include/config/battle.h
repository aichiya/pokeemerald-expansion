...
// Type settings
#define B_GHOSTS_ESCAPE             FALSE      // In Gen6+, escape prevention fails on Ghost-types. Additionally, attempting to escape from a wild battle is always sucessful.
#define B_PARALYZE_ELECTRIC         FALSE      // In Gen6+, Electric-types can no longer be paralyzed.
#define B_POWDER_GRASS              FALSE      // In Gen6+, Grass-types are immune to powder and spore moves.
#define B_UPDATED_TYPE_MATCHUPS     GEN_LATEST // Changes type matchups. src/data/types_info.h for details.
#define B_PRANKSTER_DARK_TYPES      FALSE      // In Gen7+, Prankster-elevated status moves do not affect Dark-types.
...
#define B_FLAG_AI_VS_AI_BATTLE      FLAG_SMART_WILD_AI_FLAG // If this flag is set, the player's Pokémon will be controlled by the AI in the next battle.
