...
static s32 AI_CheckBadMove(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (((GetMoveType(move) == TYPE_NEW_FIRE && GetMovePower(move) != 0) || CanBurnHitThaw(move)) && effectiveness < UQ_4_12(2.0) && (gBattleMons[battlerDef].status1 & (STATUS1_FROSTBITE | STATUS1_FREEZE)))
    {
        u32 aiMove;
        for (i = 0; i < MAX_MON_MOVES; i++)
        {
    ...
    // other
        ...
        case EFFECT_SOAK:
        {
            ...
            if (PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove)
              || (types[0] == TYPE_NEW_WATER && types[1] == TYPE_NEW_WATER && types[2] == TYPE_MYSTERY))
    ...
    if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
    {
        ...
        if (!partnerProtecting && moveTarget != MOVE_TARGET_BOTH && !DoesBattlerIgnoreAbilityChecks(battlerAtk, aiData->abilities[battlerAtk], move))
        {
            ...
            case ABILITY_WATER_COMPACTION:
                if (moveType == TYPE_NEW_WATER && GetNoOfHitsToKOBattler(battlerAtk, battlerDef, AI_THINKING_STRUCT->movesetIndex, AI_ATTACKING) >= 4)
                {
...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_HEAL_BLOCK:
        ...
        break;
    case EFFECT_SOAK:
        if (HasMoveWithType(battlerAtk, TYPE_NEW_ELECTRIC) || HasMoveWithType(battlerAtk, TYPE_NEW_NATURE) || (HasMoveEffect(battlerAtk, EFFECT_SUPER_EFFECTIVE_ON_ARG) && GetMoveArgType(move) == TYPE_NEW_WATER) )
