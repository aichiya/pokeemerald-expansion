...
static u64 GetAiFlags(u16 trainerId, enum BattlerId battler)
{
    ...
    else if (trainerId == TRAINER_SECRET_BASE || (gBattleTypeFlags & BATTLE_TYPE_SECRET_BASE))
    {
        flags = AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT | AI_FLAG_SMART_TERA;
    }
    else
    {
...
//            flags = AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_OMNISCIENT | AI_FLAG_SMART_SWITCHING | AI_FLAG_SMART_MON_CHOICES | AI_FLAG_PP_STALL_PREVENTION | AI_FLAG_SMART_TERA | AI_FLAG_RANDOMIZE_SWITCHIN;
    ...
    if (moveTarget != TARGET_USER)
    {
        ...
        if (hasTwoOpponents)
        {
            ...
            case ABILITY_SWEET_VEIL:
                ...
                break;
            case ABILITY_FLOWER_VEIL:
                if ((IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_NATURE)) && (IsNonVolatileStatusMove(move) || IsStatLoweringMove(move)))
                    RETURN_SCORE_MINUS(10);
                break;
            case ABILITY_AROMA_VEIL:
                if (IsAromaVeilProtectedEffect(moveEffect))
...
// the following checks apply to any target (including user)
    ...
    case EFFECT_HELPING_HAND:
        if (!hasPartner
          ...
          || aiData->abilities[GetPartnerBattler(battlerAtk)] == ABILITY_GREAT_BLOOMING
          || aiData->abilities[GetPartnerBattler(battlerAtk)] == ABILITY_SECOND_ADVENT
          || aiData->abilities[GetPartnerBattler(battlerAtk)] == ABILITY_PRISMA_ZWEI)
          || (aiData->partnerMove != MOVE_NONE && IsBattleMoveStatus(aiData->partnerMove))
...
static s32 AI_CalcMoveEffectScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, struct AiLogicData *aiData)
{
    ...
    case EFFECT_FOCUS_ENERGY:
        if (aiData->abilities[battlerAtk] == ABILITY_SUPER_LUCK
         ...
         || aiData->abilities[battlerAtk] == ABILITY_ASSASSIN_SENSE
    ...
    case EFFECT_EMBARGO:
        ...
        break;
    case EFFECT_POWDER:
        if (incomingMove != MOVE_NONE && !IsBattleMoveStatus(incomingMove) && predictedType == TYPE_NEW_FIRE)
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_TELEKINESIS:
    ...
    case EFFECT_SOAK:
        ...
        || HasMoveWithType(GetPartnerBattler(battlerAtk), TYPE_NEW_ELECTRIC) || HasMoveWithType(GetPartnerBattler(battlerAtk), TYPE_NEW_NATURE)
    ...
    for (u32 effectId = 0; effectId < additionalEffectCount; effectId++)
    {
        ...
        else // consider move effects that hinder the target
        {
            ...
            case MOVE_EFFECT_UBW:
                if (ShouldClearTerrain(battlerAtk, B_TERRAIN_UBW))
                {
                    ADJUST_SCORE(BAD_EFFECT);
                    break;
                }
                if (ShouldSetTerrain(battlerAtk, B_TERRAIN_UBW)
                 || ShouldClearTerrain(battlerAtk, gFieldTimers.terrain != B_TERRAIN_NONE))
                    ADJUST_SCORE(DECENT_EFFECT);
                break;
            case MOVE_EFFECT_DARKNESS_TERRAIN:
                if (ShouldClearTerrain(battlerAtk, B_TERRAIN_DARKNESS))
                {
                    ADJUST_SCORE(BAD_EFFECT);
                    break;
                }
                if (ShouldSetTerrain(battlerAtk, B_TERRAIN_DARKNESS)
                 || ShouldClearTerrain(battlerAtk, gFieldTimers.terrain != B_TERRAIN_NONE))
                    ADJUST_SCORE(DECENT_EFFECT);
                break;
            case MOVE_EFFECT_MIASMA_TERRAIN:
                if (ShouldClearTerrain(battlerAtk, B_TERRAIN_MIASMA))
                {
                    ADJUST_SCORE(BAD_EFFECT);
                    break;
                }
                if (ShouldSetTerrain(battlerAtk, B_TERRAIN_MIASMA)
                 || ShouldClearTerrain(battlerAtk, gFieldTimers.terrain != B_TERRAIN_NONE))
                    ADJUST_SCORE(DECENT_EFFECT);
                break;
