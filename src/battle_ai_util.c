...
bool32 AI_IsDamagedByRecoil(u32 battler)
{
    ...
    if (ability == ABILITY_MAGIC_GUARD || ability == ABILITY_FANTASY_BREAKER || ability == ABILITY_ROCK_HEAD)
        return FALSE;
    return TRUE;
}
...
bool32 IsMoveEncouragedToHit(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (B_TOXIC_NEVER_MISS >= GEN_6 && gMovesInfo[move].effect == EFFECT_TOXIC && IS_BATTLER_OF_TYPE(battlerAtk, TYPE_NEW_MIASMA))
        return TRUE;
...
bool32 ShouldTryOHKO(u32 battlerAtk, u32 battlerDef, u32 atkAbility, u32 defAbility, u32 move)
{
    ...
    else    // test the odds
    {
        ...
        if (B_SHEER_COLD_ACC >= GEN_7 && move == MOVE_SHEER_COLD && !IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_NEW_ICE))
            odds -= 10;
...
bool32 ShouldSetRain(u32 battlerAtk, u32 atkAbility, u32 holdEffect)
{
    ...
    if (holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA
     && (atkAbility == ABILITY_SWIFT_SWIM
      ...
      || HasMoveWithType(battlerAtk, TYPE_NEW_WATER)))
    {
        return TRUE;
    }
    return FALSE;
...
bool32 ShouldSetSun(u32 battlerAtk, u32 atkAbility, u32 holdEffect)
{
    ...
    if (holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA
     && (atkAbility == ABILITY_CHLOROPHYLL
      ...
      || HasMoveWithType(battlerAtk, TYPE_NEW_FIRE)))
    {
        return TRUE;
    }
    return FALSE;
...
{
    if (!IS_BATTLER_OF_TYPE(battlerId, TYPE_NEW_EARTH)
      && !IS_BATTLER_OF_TYPE(battlerId, TYPE_NEW_STEEL)
...
u32 GetBattlerSecondaryDamage(u32 battlerId)
{
    ...
    if (AI_DATA->abilities[battlerId] == ABILITY_MAGIC_GUARD || AI_DATA->abilities[battlerId] == ABILITY_FANTASY_BREAKER)
        return FALSE;
...
static bool32 PartyBattlerShouldAvoidHazards(u32 currBattler, u32 switchBattler)
{
    ...
    if (flags & SIDE_STATUS_SPIKES && ((type1 != TYPE_NEW_FLYING && type2 != TYPE_NEW_FLYING
...
bool32 AI_CanPoison(u32 battlerAtk, u32 battlerDef, u32 defAbility, u32 move, u32 partnerMove)
{
    if (!CanBePoisoned(battlerAtk, battlerDef, GetBattlerAbility(battlerDef))
      ...
      || PartnerMoveEffectIsStatusSameTarget(BATTLE_PARTNER(battlerAtk), battlerDef, partnerMove))
        return FALSE;
    else if (defAbility != ABILITY_CORROSION && (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_MIASMA) || IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_STEEL)))
        return FALSE;
...
bool32 PartnerMoveEffectIsTerrain(u32 battlerAtkPartner, u32 partnerMove)
{
    ...
    if (partnerMove != MOVE_NONE
     && (gMovesInfo[partnerMove].effect == EFFECT_GRASSY_TERRAIN
      ...
      || gMovesInfo[partnerMove].effect == EFFECT_UBW))
        return TRUE;
