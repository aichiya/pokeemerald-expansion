...
bool32 IsBattlerTrapped(enum BattlerId battlerAtk, enum BattlerId battlerDef)
{
    ...
    if (AI_IsAbilityOnSide(battlerAtk, ABILITY_MAGNET_PULL)
        && IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_METAL))
        return TRUE;
...
bool32 Ai_IsPriorityBlocked(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, struct AiLogicData *aiData)
{
    ...
    if (IsDoubleBattle() && IsDazzlingAbility(aiData->abilities[GetPartnerBattler(battlerDef)]) && !IsAbilityOnField(ABILITY_TIME_JACKER))
        return TRUE;
...
bool32 CanEndureHit(enum BattlerId battler, enum BattlerId battlerTarget, enum Move move)
{
    ...
    if ((gAiLogicData->abilities[battler] == ABILITY_PARENTAL_BOND 
      || gAiLogicData->abilities[battler] == ABILITY_TWIN_BODY 
      || gAiLogicData->abilities[battler] == ABILITY_SIBLINGS_BOND)
...
u32 AI_GetSwitchinTerrain(enum BattlerId battler)
{
    ...
    case ABILITY_BLADE_WORKS:
        newTerrain = B_TERRAIN_UBW;
        break;
    case ABILITY_DARKNESS_SURGE:
        newTerrain = B_TERRAIN_DARKNESS;
        break;
    case ABILITY_MIASMA_SURGE:
        newTerrain = B_TERRAIN_MIASMA;
        break;
    default:
        return gFieldTimers.terrain;
    }
...
{
    enum Move predictedMove = GetPredictedMove(battlerAtk, battlerDef, gAiLogicData);
    if (((!IsMoldBreakerTypeAbility(battlerAtk, gAiLogicData->abilities[battlerAtk], move) && (defAbility == ABILITY_SHIELD_DUST || defAbility == ABILITY_INNER_FOCUS || defAbility == ABILITY_ADVENT || defAbility == ABILITY_PRISMA_ZWEI || defAbility == ABILITY_FLAWLESS))
...
bool32 AnyPartyMemberStatused(enum BattlerId battlerId, bool32 checkSoundproof)
{
    ...
    if (HasPartner(battlerId))
    {
        ...
        if ((GetConfig(B_HEAL_BELL_SOUNDPROOF) == GEN_5
            ...
            || gAiLogicData->abilities[GetPartnerBattler(battlerId)] != ABILITY_LAST_CADENZA
            || !checkSoundproof)
...
static bool32 AI_CanStatChangeBePrevented(struct BattleCalcValues *cv)
{
    ...
    default:
        if (IsSubstituteProtected(cv->battlerAtk, cv->battlerDef, cv->abilities[cv->battlerAtk], cv->move)
         ...
         || cv->abilities[cv->battlerDef] == ABILITY_GREAT_BLOOMING
         || cv->abilities[cv->battlerDef] == ABILITY_SECOND_ADVENT
         || cv->abilities[cv->battlerDef] == ABILITY_PRISMA_ZWEI))
            return TRUE;
    }
