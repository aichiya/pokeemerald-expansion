...
static void WallyHandleDrawTrainerPic(enum BattlerId battler)
{
    BtlController_HandleDrawTrainerPic(battler, TRAINER_PIC_STEVEN, FALSE,
                                       80, 80 + 4 * (8 - GetTrainerBackPicCoords(TRAINER_PIC_STEVEN)->size),
                                       30);
}

static void WallyHandleTrainerSlide(enum BattlerId battler)
{
    BtlController_HandleTrainerSlide(battler, TRAINER_PIC_STEVEN);
}
...
static void WallyHandleIntroTrainerBallThrow(enum BattlerId battler)
{
    const u16 *trainerPal = GetTrainerBackPicPalette(TRAINER_PIC_STEVEN);
