...
static bool32 HandleEndTurnGraceOfDream(enum BattlerId battler)
{
     ...
     && IsBattlerAlive(battler))
    {
        ...
        if (validToRaise) // Can raise one stat
        {
            ...
            {
                stat = RandomUniformExcept(RNG_GRACE_OF_DREAM_INCREASE, STAT_ATK, statsNum - 1, MoodyCantRaiseStat);
                SetStatChange(battler, stat, 2);
            }
            BattleScriptCall(BattleScript_GraceOfDreamStatRaised);
            effect = TRUE;
        }
    }
...
static bool32 HandleEndTurnLeechSeed(enum BattlerId battler)
{
     ...
     && !(IsAbilityAndRecord(drainedBattler, GetBattlerAbility(drainedBattler), ABILITY_MAGIC_GUARD) || IsAbilityAndRecord(drainedBattler, GetBattlerAbility(drainedBattler), ABILITY_FANTASY_BREAKER)))
    {
        ...
        if (GetBattlerAbility(drainedBattler) == ABILITY_LIQUID_OOZE || GetBattlerAbility(drainedBattler) == ABILITY_FANTASY_BREAKER)
        {
...
static bool32 HandleEndTurnPerishSong(enum BattlerId battler)
{
    ...
    if (gBattleMons[battler].volatiles.perishSong
     && IsBattlerPresent(battler)
     && !IsAbilityAndRecord(battler, GetBattlerAbility(battler), ABILITY_FANTASY_BREAKER))
    {
    ...
    case THIRD_EVENT_BLOCK_UPROAR:
        if (gBattleMons[battler].volatiles.uproarTurns)
        {
            for (gEffectBattler = 0; gEffectBattler < gBattlersCount; gEffectBattler++)
            {
                ...
                bool32 hasSoundproof = GetConfig(B_UPROAR_IGNORE_SOUNDPROOF) < GEN_5 && (GetBattlerAbility(gEffectBattler) == ABILITY_SOUNDPROOF || GetBattlerAbility(gEffectBattler) == ABILITY_LAST_CADENZA);
