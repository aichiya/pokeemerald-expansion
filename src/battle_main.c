...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    [TRAINER_CLASS_0XF5] = { _("Professor") },
    [TRAINER_CLASS_0XF6] = { _("Zero Trainer Origin") },
    [TRAINER_CLASS_0XF7] = { _("Ordinary Magician") },
    ...
    case EFFECT_WEATHER_BALL:
        if (state == MON_IN_BATTLE)
        {
            ...
            if (weather & B_WEATHER_SUN)
                ...
                return TYPE_ICE;
            else if (weather & B_WEATHER_EX_SHADOW_SKY)
...
void SetTypeBeforeUsingMove(enum Move move, enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect)
{
    ...
    if ((gFieldStatuses & STATUS_FIELD_ION_DELUGE && moveType == TYPE_NEW_ILLUSION)
     || gBattleMons[battler].volatiles.electrified)
        gBattleStruct->dynamicMoveType = TYPE_NEW_ELECTRIC;
