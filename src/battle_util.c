...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 move, u32 moveType, enum AbilityEffectOptions option)
{
    ...
    case ABILITY_VOLT_ABSORB:
        if (moveType == TYPE_NEW_ELECTRIC && GetMoveTarget(move) != MOVE_TARGET_ALL_BATTLERS && gCurrentMove != MOVE_01_CRUSHER)
    ...
    case ABILITY_MOTOR_DRIVE:
        if (moveType == TYPE_NEW_ELECTRIC && GetMoveTarget(move) != MOVE_TARGET_ALL_BATTLERS && gCurrentMove != MOVE_01_CRUSHER) // Potential bug in singles (might be solved with simu hp reudction)
        {
    ...
    case ABILITY_LIGHTNING_ROD:
        if (B_REDIRECT_ABILITY_IMMUNITY >= GEN_5 && moveType == TYPE_NEW_ELECTRIC && GetMoveTarget(move) != MOVE_TARGET_ALL_BATTLERS && gCurrentMove != MOVE_01_CRUSHER) // Potential bug in singles (might be solved with simu hp reudction)
        {
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            statId = STAT_SPATK;
        }
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, bool32 recordAbilities, uq4_12_t modifier, u32 defAbility)
{
    ...
    else if (moveType == TYPE_NEW_EARTH && !IsBattlerGroundedInverseCheck(battlerDef, TRUE) && !(MoveIgnoresTypeIfFlyingAndUngrounded(move)))
    {
        modifier = UQ_4_12(0.0);
