...
bool32 CanTargetBlockPranksterMove(struct DamageContext *ctx, s32 movePriority)
{
    if (movePriority <= 0
     ...
     || (IsBattleMoveStatus(ctx->move) && (ctx->abilities[ctx->battlerDef] == ABILITY_MAGIC_BOUNCE || ctx->abilities[ctx->battlerDef] == ABILITY_FANTASY_BREAKER || gProtectStructs[ctx->battlerDef].bounceMove)))
        return FALSE;
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, enum BattlerId battler, enum Ability ability, enum Move move, bool32 shouldAbilityTrigger)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_MANA_DISTURPTOR:
            ...
            {
                gEffectBattler = battler;
                gBattleStruct->manaDisturptorActivated = TRUE;
                for (enum BattlerId i = 0; i < gBattlersCount; i++)
                {
                    if (IsBattlerAlly(battler, i) || !IsBattlerAlive(i))
                        continue;
        ...
        case ABILITY_MISFORTUNE_AURA:
            ...
            {
                gEffectBattler = battler;
                gBattleStruct->misfortuneAuraActivated = TRUE;
                for (enum BattlerId i = 0; i < gBattlersCount; i++)
                {
                    if (IsBattlerAlly(battler, i) || !IsBattlerAlive(i))
                        continue;
                    SetStatChange(i, STAT_ACC, -1);
                }
        ...
        case ABILITY_LUNACY:
            if(shouldAbilityTrigger)
            {
                ...
                if (validToRaise) // Can raise one stat
                {
                    if (validToRaise) // Find stat to raise
                    {
                        stat = RandomUniformExcept(RNG_LUNACY_INCREASE, STAT_ATK, statsNum - 1, MoodyCantRaiseStat);
    ...
    case ABILITYEFFECT_ENDTURN:
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_SHICHININ_MISAKI:
                if (CompareStat(battler, STAT_EVASION, MAX_STAT_STAGE, CMP_LESS_THAN, gLastUsedAbility) && !BattlerJustSwitchedIn(battler))
                {
                    gEffectBattler = gBattlerAbility = battler;
                    SetStatChange(battler, STAT_EVASION, -1);
                    BattleScriptCall(BattleScript_AbilityStatChange);
                    effect++;
                }
                break;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_ULTRA_EGO:
             ...
             && IsBattlerAlive(gBattlerTarget))
            {
                ...
                if (validToRaise) // Find stat to raise
                {
                    stat = RandomUniformExcept(RNG_ULTRA_EGO_INCREASE, STAT_ATK, statsNum - 1, MoodyCantRaiseStat);
                    SetStatChange(battler, stat, 12);
                }
                gEffectBattler = gBattlerAbility = gBattlerTarget;
                BattleScriptCall(BattleScript_AbilityStatChange);
                effect++;
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_ULTRA_FORM_MARCH:
            if (IsBattlerAlive(gBattlerTarget)
             && !gBattleStruct->unableToUseMove
             && RandomChance(RNG_ULTRA_MARCH, 1, 3)
             && IsBattlerTurnDamaged(gBattlerTarget, EXCLUDING_SUBSTITUTES)
             && CanBeParalyzed(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerTarget))
             && !MoveHasAdditionalEffect(gCurrentMove, MOVE_EFFECT_FLINCH)
    ...
    case ABILITYEFFECT_MOVE_END_OTHER: // Abilities that activate on *another* battler's moveend: Dancer, Soul-Heart, Receiver, Symbiosis
        ...
        case ABILITY_GRAND_THEORY:
            ...
            break;
        case ABILITY_DOUR_ECHO: // Decrease opposing mons' higher attacking stat
            if (IsBattlerAlive(battler)
             ...
             && !IsOpposingSideEmpty(battler))
            {
                // gEffectBattler = battler;
                gEffectBattler = gBattlerAbility = battler;
                gBattleMons[battler].volatiles.activateEchoAbilitiesDour = FALSE;
                for (enum BattlerId i = 0; i < gBattlersCount; i++)
                {
                    if (IsBattlerAlly(battler, i) || !IsBattlerAlive(i))
                        continue;

                    if (gBattleMons[i].attack > gBattleMons[i].spAttack)
                    {
                        SetStatChange(i, STAT_ATK, -1);
                    }
                    else if (gBattleMons[i].attack < gBattleMons[i].spAttack)
                    {
                        SetStatChange(i, STAT_SPATK, -1);
                    }
                    else
            ...
            break;
        case ABILITY_MANIC_ECHO: // Increase the user's higher attacking stat
            if (IsBattlerAlive(battler)
             && IsSoundMove(gCurrentMove))
            {
                gBattleMons[battler].volatiles.activateEchoAbilitiesManic = FALSE;
                gEffectBattler = gBattlerAbility = battler;
                if (gBattleMons[battler].attack > gBattleMons[battler].spAttack)
                {
                    SetStatChange(battler, STAT_SPATK, 1);
                }
                else if (gBattleMons[battler].attack < gBattleMons[battler].spAttack)
                {
                    SetStatChange(battler, STAT_ATK, 1);
                }
                else
        ...
        case ABILITY_LAST_CADENZA:
             ...
             && IsSoundMove(gCurrentMove))
            {
                // gEffectBattler = battler;
                gEffectBattler = gBattlerAbility = battler;
                gBattleMons[battler].volatiles.activateEchoAbilitiesLastCadenza = FALSE;
                for (enum BattlerId i = 0; i < gBattlersCount; i++)
                {
                    enum BattlerId oppositeBattler = GetOppositeBattler(battler);
                    enum BattlerId oppositeBattlerPartner = GetPartnerBattler(oppositeBattler);
                    ...
                    if (i == oppositeBattler || i == oppositeBattlerPartner)
                    {
                        if (gBattleMons[i].attack > gBattleMons[i].spAttack)
                        {
                            SetStatChange(i, STAT_ATK, -1);
        ...
        case ABILITY_MAGICIAN:
             ...
             && !gSpecialStatuses[battler].gemBoost) // In base game, gems are consumed after magician would activate.
            {
                ...
                for (u32 i = 0; i < gBattlersCount; i++)
                {
                    ...
                    if (GetBattlerAbility(targetBattler) == ABILITY_STICKY_HOLD || GetBattlerAbility(targetBattler) == ABILITY_COLLECTOR)
                    {
                        effect = FALSE;
                        break;
                    }
...
bool32 IsUBWTerrainAffected(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect, enum BattleTerrain currTerrain)
{
    return IsBattlerTerrainAffected(battler, ability, holdEffect, B_TERRAIN_UBW, currTerrain);
}

bool32 IsDarknessTerrainAffected(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect, enum BattleTerrain currTerrain)
{
    return IsBattlerTerrainAffected(battler, ability, holdEffect, B_TERRAIN_DARKNESS, currTerrain);
}

bool32 IsMiasmaTerrainAffected(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect, enum BattleTerrain currTerrain)
{
    return IsBattlerTerrainAffected(battler, ability, holdEffect, B_TERRAIN_MIASMA, currTerrain);
}
...
bool32 CanSetNonVolatileStatus(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum MoveEffect effect, enum ResultOption option)
{
    ...
    case MOVE_EFFECT_FROSTBITE:
        ...
        else if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_ICE) || IsBattlerWeatherAffected(GetBattlerHoldEffect(battlerDef), GetWeather(), B_WEATHER_SUN))
        {
            battleScript = BattleScript_NotAffected;
        }
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    case EFFECT_SOLAR_BEAM:
    {
        ...
        if ((GetConfig(B_SANDSTORM_SOLAR_BEAM) >= GEN_3 && weather & B_WEATHER_LOW_LIGHT)
            || weather & (B_WEATHER_RAIN | B_WEATHER_ICY_ANY | B_WEATHER_FOG | B_WEATHER_EX_SHADOW_SKY)) // Excludes Sandstorm
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    case ABILITY_AUTUMN_GODDESSES: // Aki Sisters
        if (IsBattlerWeatherAffected(ctx->holdEffects[ctx->battlerAtk], ctx->weather, B_WEATHER_SUN))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    ...
    case ABILITY_WINTER_GIFT:
        if ((IsBattlerWeatherAffected(ctx->holdEffects[ctx->battlerAtk], ctx->weather, B_WEATHER_SNOW) 
         || IsBattlerWeatherAffected(ctx->holdEffects[ctx->battlerAtk], ctx->weather, B_WEATHER_HAIL)) 
         && IsBattleMoveSpecial(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
...
static inline u32 CalcDefenseStat(struct DamageContext *ctx)
{
    ...
    if (GetConfig(B_SANDSTORM_SPDEF_BOOST) >= GEN_4
	 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_EARTH)
	 && GetAttackerWeather(ctx->holdEffects[ctx->battlerAtk], ctx->abilities[ctx->battlerAtk], ctx->weather) & B_WEATHER_SANDSTORM
	 && !usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
    // snow def boost for ice types
    if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NEW_ICE)
	 && (GetAttackerWeather(ctx->holdEffects[ctx->battlerAtk], ctx->abilities[ctx->battlerAtk], ctx->weather) & B_WEATHER_SNOW || GetAttackerWeather(ctx->holdEffects[ctx->battlerAtk], ctx->abilities[ctx->battlerAtk], ctx->weather) & B_WEATHER_HAIL)
	 && usesDefStat)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
...
static inline uq4_12_t GetDefenderAbilitiesModifier(struct DamageContext *ctx)
{
    ...
    case ABILITY_PRISM_ARMOR:
        ...
        break;
    case ABILITY_FLUFFY:
        if (ctx->moveType == TYPE_NEW_FIRE && !IsMoveMakingContact(ctx->battlerAtk, ctx->battlerDef, ctx->abilities[ctx->battlerAtk], ctx->holdEffects[ctx->battlerAtk], ctx->move))
        {
...
s32 CalcCritChanceStage(struct DamageContext *ctx)
{
    ...
    else
    {
        critChance  = (gBattleMons[ctx->battlerAtk].volatiles.focusEnergy != 0 ? 2 : 0)
                    ...
                    + (ctx->abilities[ctx->battlerAtk] == ABILITY_ASSASSIN_SENSE ? 1 : 0)
    ...
    if (critChance != CRITICAL_HIT_BLOCKED && (ctx->abilities[ctx->battlerDef] == ABILITY_BATTLE_ARMOR || ctx->abilities[ctx->battlerDef] == ABILITY_SHELL_ARMOR || ctx->abilities[ctx->battlerDef] == ABILITY_GUARD_ARMOR))
    {
...
s32 CalcCritChanceStageGen1(struct DamageContext *ctx)
{
    ...
    if (ctx->abilities[ctx->battlerAtk] == ABILITY_SUPER_LUCK || ctx->abilities[ctx->battlerAtk] == ABILITY_ASSASSIN_SENSE)
        critChance *= 4;

    if (critChance > 255)
        critChance = 255;

    if (ctx->abilities[ctx->battlerDef] == ABILITY_BATTLE_ARMOR || ctx->abilities[ctx->battlerDef] == ABILITY_SHELL_ARMOR || ctx->abilities[ctx->battlerDef] == ABILITY_GUARD_ARMOR)
    {
        if (ctx->updateFlags)
            RecordAbilityBattle(ctx->battlerDef, ctx->abilities[ctx->battlerDef]);
        critChance = CRITICAL_HIT_BLOCKED;
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    if (ignoreTypeCalc && ctx->move != MOVE_THUNDER_WAVE)
    {
    ...
    else if (ctx->moveType == TYPE_NEW_EARTH
        ...
        && !(ctx->holdEffects[ctx->battlerDef] == HOLD_EFFECT_RING_TARGET && IS_BATTLER_OF_TYPE(ctx->battlerDef, TYPE_NEW_FLYING) && !IsBattlerUngroundedByAbilityItemOrEffect(ctx->battlerDef, ctx->abilities[ctx->battlerDef], ctx->holdEffects[ctx->battlerDef])))
    {
        modifier = UQ_4_12(0.0);
    ...
    if ((((ctx->abilities[ctx->battlerDef] == ABILITY_WONDER_GUARD || ctx->abilities[ctx->battlerDef] == ABILITY_PLAY_GHOST) && modifier <= UQ_4_12(1.0) && !isPresentHealing)
