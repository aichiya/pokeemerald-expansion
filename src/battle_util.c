...
static inline uq4_12_t GetDefenderAbilitiesModifier(struct DamageContext *ctx)
{
    ...
    case ABILITY_PRISM_ARMOR:
        ...
        break;
    case ABILITY_FLUFFY:
        if (!IsMoveMakingContact(ctx->move, ctx->battlerAtk) && ctx->moveType == TYPE_NEW_FIRE)
        {
            modifier = UQ_4_12(2.0);
            recordAbility = TRUE;
        }
        else if (IsMoveMakingContact(ctx->move, ctx->battlerAtk) && ctx->moveType != TYPE_NEW_FIRE)
        {
            modifier = UQ_4_12(0.5);
            recordAbility = TRUE;
        }
...
static inline void MulByTypeEffectiveness(struct DamageContext *ctx, uq4_12_t *modifier, u32 defType)
{
    ...
    else if ((ctx->moveType == TYPE_NEW_DREAM || ctx->moveType == TYPE_NEW_ILLUSION) && defType == TYPE_NEW_NETHER && gBattleMons[ctx->battlerDef].status2 & STATUS2_FORESIGHT && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
    }
    ...
    if (ctx->moveType == TYPE_NEW_REASON && defType == TYPE_NEW_DARK && gStatuses3[ctx->battlerDef] & STATUS3_MIRACLE_EYED && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    else if (B_SHEER_COLD_IMMUNITY >= GEN_7 && GetMoveEffect(ctx->move) == EFFECT_SHEER_COLD && IS_BATTLER_OF_TYPE(ctx->battlerDef, TYPE_NEW_ICE))
    {
        modifier = UQ_4_12(0.0);
    }
