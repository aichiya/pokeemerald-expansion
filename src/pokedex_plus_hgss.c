...
static void PrintMonInfo(u32 num, u32 value, u32 owned, u32 newEntry)
{
    ...
    if (owned)
    {
    	PrintDecimalNumHeight(0, gPokedexEntries[num].height, 186, 64); //HGSS_Ui
    	PrintDecimalNumWeight(0, gPokedexEntries[num].weight, 186, 77); //HGSS_Ui
    }
    else
    {
        PrintInfoScreenText(gText_UnkHeight, 186, 64); //HGSS_Ui
        PrintInfoScreenText(gText_UnkWeight, 186, 77); //HGSS_Ui
    }
    if (owned)
    {
